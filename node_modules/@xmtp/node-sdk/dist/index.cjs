...
var contentTypeGroupUpdated = require('@xmtp/content-type-group-updated');
...
class DecodedMessage {
    ...
    constructor(client, message) {
        ...
        this.contentType = message.content.type
            ? new contentTypePrimitives.ContentTypeId(message.content.type)
            : undefined;
...
class Client {
    ...
    static async create(accountAddress, options) {
        ...
        return new Client(await nodeBindings.createClient(host, isSecure, dbPath, inboxId, accountAddress, options?.encryptionKey, options?.requestHistorySync, options?.logging ?? "off"), [new contentTypeGroupUpdated.GroupUpdatedCodec(), new contentTypeText.TextCodec(), ...(options?.codecs ?? [])]);
    }
    get accountAddress() {
    ...
    decodeContent(message, contentType) {
        ...
        if (contentType.sameAs(contentTypeGroupUpdated.ContentTypeGroupUpdated) &&
...
const generateInboxId = (accountAddress) => {
    return nodeBindings.generateInboxId(accountAddress);
};
const getInboxIdForAddress = async (accountAddress, env = "dev") => {
    const host = ApiUrls[env];
    const isSecure = host.startsWith("https");
    return nodeBindings.getInboxIdForAddress(host, isSecure, accountAddress);
};
...
exports.generateInboxId = generateInboxId;
exports.getInboxIdForAddress = getInboxIdForAddress;
